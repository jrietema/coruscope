class Admin::Cms::FilesController < Admin::Cms::BaseController
  ...
  def index
    @files = Cms::File.includes(:categories).for_category(params[:category]).where(['site_id IN (?)', @site.mirrors.map(&:id)]).order('cms_files.position')
...
protected
  ...
  def build_file
    @file = @site.original_mirror.files.new
  end
  
  def load_file
    site_ids = @site.mirrors.map(&:id)
    @file = Cms::File.where(['id = ? AND site_id IN (?)', params[:id], site_ids]).first
